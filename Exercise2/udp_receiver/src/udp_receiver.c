...
int main(int argc, char **argv) {
    ...
    while ((opt = getopt(argc, argv, "p:s:r:h")) != -1) {
        switch (opt) {
            ...
            case 's':
                ...
                CHECK(buffer_size, TAG, "Invalid size: %s (must be between 28-65507)", optarg);
                break;
            case 'r':
                ...
                break;
            case 'h':
                fprintf(stderr, "%s [-p <port 1-65535>] [-s <buffer_size>] [-h (this message)]\n", TAG);
                return EXIT_SUCCESS;
            case '?':
    ...
    do {
        ...
        ssize_t bytes_received = recvfrom(udp_rx_socket, rx_buf, buffer_size - 1, 0,
